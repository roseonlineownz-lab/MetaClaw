...
```
(선택) WeChat 개인 계정 브리지([Node.js](https://nodejs.org/)가 PATH에 있어야 하며 SDK는 22 이상을 권장).

- **OpenClaw 원클릭 플러그인**으로 설치한 경우 WeChat용 Node 의존성이 자동 설치됩니다.
- **pip**으로 설치한 경우 `pip install` 후 한 번 Node 의존성을 설치하세요:
...
설정에 `wechat.bridge_dir`을 지정했다면 해당 디렉터리에서 `npm install`을 실행하세요. 전체 설정(QR 로그인, 재로그인, 문제 해결): [`metaclaw/wechat_node/README.md`](../metaclaw/wechat_node/README.md).
